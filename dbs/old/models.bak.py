...
from sqlalchemy import (Column, Integer, String, ForeignKey,
                        ...
                        UniqueConstraint, Table, ForeignKeyConstraint)
...
class Case(AuditMixin, Model):
    ...
    person2 = relationship(u'Person', secondary=u't_case_witness', backref=u'person_person_person_person_person_person_cases')
    person3 = relationship(u'Person', secondary=u't_case_judge', backref=u'person_person_person_person_person_person_cases_0')
    offense = relationship(u'Offense', secondary=u't_case_offense', backref=u'cases')
    person4 = relationship(u'Person', secondary=u't_case_prosecutor', backref=u'person_person_person_person_person_person_cases')
    person5 = relationship(u'Person', secondary=u't_case_policeman', backref=u'person_person_person_person_person_person_cases_0')
...
class CaseHearing(AuditMixin, Model):
    ...
    case_id =  Column(Integer, primary_key=True, nullable=False)
        #Column(Integer, autoincrement=True, primary_key=True, nullable=False)
    #
    case_case_name = Column(String(150), primary_key=True, nullable=False)
    case_case_number = Column(String(30), primary_key=True, nullable=False)
...
class Court(RefTypeMixin, PlaceMixin, ContactMixin, Model):
    __tablename__ = 'court'

    #id = Column(Integer, primary_key=True, server_default=FetchedValue())
    id = Column(Integer, autoincrement=True, primary_key=True, nullable=False)

    town_fk = Column(ForeignKey(u'town.id'), nullable=False, index=True)
    town = relationship(u'Town', primaryjoin='Court.town == Town.id', backref=u'courts')

    court_level_fk = Column(ForeignKey(u'courtlevel.id'), nullable=False, index=True)
...
class Offense(RefTypeMixin, Model):
...
class Policestation(RefTypeMixin, Model):
    __tablename__ = 'policestation'
    id = Column(Integer, autoincrement=True, primary_key=True, nullable=False)
    #id = Column(Integer, primary_key=True, server_default=FetchedValue())
    town_fk = Column(ForeignKey(u'town.id'), nullable=False, index=True)
    town = relationship(u'Town', primaryjoin='Policestation.town == Town.id', backref=u'policestations')


class Prison(RefTypeMixin, Model):
    __tablename__ = 'prison'
    id = Column(Integer, autoincrement=True, primary_key=True, nullable=False)
    #id = Column(Integer, primary_key=True, server_default=FetchedValue())

    town_fk = Column(ForeignKey(u'town.id'), nullable=False, index=True)
    town = relationship(u'Town', primaryjoin='Prison.town == Town.id', backref=u'prisons')
    warden = Column(String(50), nullable=False)
...
class District(RefTypeMixin, Model):
    ...
    #id = Column(Integer, primary_key=True, server_default=FetchedValue())
    region_fk = Column(ForeignKey(u'region.id'), nullable=False, index=True)
    region = relationship(u'Region', primaryjoin='District.region == Region.id', backref=u'districts')
...
class Town(RefTypeMixin, Model):
    ...
    district_fk = Column(ForeignKey(u'district.id'), nullable=False, index=True)
    district = relationship(u'District', primaryjoin='Town.district == District.id', backref=u'towns')


class Person(PersonMixin, ContactMixin, AuditMixin, Model):
    ...
    id = Column(Integer, autoincrement=True, primary_key=True, nullable=False)
    #id = Column(Integer, primary_key=True, server_default=FetchedValue())
    gender_fk = Column(ForeignKey(u'gender.id'), nullable=False, index=True)
    gender = relationship(u'Gender', primaryjoin='Person.gender == Gender.id', backref=u'people')

    date_of_birth = Column(Date, nullable=False)
    identity = Column(Text, nullable=False)
    address = Column(Text, nullable=False)
    biometrics = Column(Text, nullable=False)
    classtype = Column(Text, nullable=False)
    with_others = Column(Boolean)
    remanded = Column(Boolean)

    court_fk = Column(ForeignKey(u'court.id'), index=True)
    court = relationship(u'Court', primaryjoin='Person.court == Court.id', backref=u'people')

    court_level_fk = Column(ForeignKey(u'courtlevel.id'), index=True)
