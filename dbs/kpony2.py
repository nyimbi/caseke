...
class Filing(db.Entity):
    ...
    TotalFees = Required(float, default="0.00")
    causes_of_action = Set(CauseOfAction)
    ...
    AmountPaid = Required(float, default="0.00")
    FeeBalance = Required(float, default="0.00")
    PaymentHistory = Required(LongStr)
    doc_templates = Set('DocTemplate')
    DocType = Required(str, 100)
    Doc = Required(LongStr)
    DocBin = Required(LongStr)
    DocThumbNail = Required(buffer)
    DocJSON = Required(Json)
    PageCount = Required(int)
    BinHash = Required(str, 100)
    TextHash = Required(str, 100)


class FilingType(db.Entity):
    id = PrimaryKey(int, auto=True)
    Cost = Required(float, default="0.00")
    filings = Set(Filing)
    PerPageCost = Required(float)


class DocTemplate(db.Entity):
    id = PrimaryKey(int, auto=True)
    Template = Required(LongStr)
    TemplateJson = Required(Json)
    filings = Set(Filing)
