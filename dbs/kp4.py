...
class Case(db.Entity):
    ...
    tags = Set('Tag')
    reported_to = Optional('PolOfficer', reverse='case_reports')
    case_investigators = Set('CaseInvestigation')
...
class Defendant(db.Entity):
    ...
    medical_events = Set('MedEvent', nullable=True)
    visits = Set('Visit')
...
Rank, ServiceNumber, Specialization, Notes"""
    ...
    case_reports = Set(Case, reverse='reported_to')
    case_investigations = Set('CaseInvestigation')
    PostDate = Optional(date, nullable=True)
...
class Hearing(db.Entity):
    ...
    witnesses = Set(Witness)
    RemandedToPrison = Set('PrisonCommital', nullable=True, cascade_delete=False)
    ...
    bail = Set('Bail', nullable=True, cascade_delete=False)
...
class CaseInvestigation(db.Entity):
    pol_officers = Required(PolOfficer)
    cases = Required(Case)
    PrimaryKey(pol_officers, cases)
