...
from sqlalchemy.dialects.postgresql import (
    ...
    ENUM, FLOAT, HSTORE, INET, INTEGER, INTERVAL, JSON, JSONB, MACADDR,
    NUMERIC, OID, REAL, SMALLINT, TEXT, TIME, TIMESTAMP, UUID, VARCHAR,
    INT4RANGE, INT8RANGE, NUMRANGE, DATERANGE, TSRANGE, TSTZRANGE, TSVECTOR)
...
from sqlalchemy import (Column, Integer, String, ForeignKey, Sequence, Float,
                        ...
                        ForeignKeyConstraint, Numeric, LargeBinary, Table)
...
class Case(ActivityMixin, AuditMixin, NameMixin, Model):
    ...
    prosecutor = relationship(
        u'Prosecutor',
        secondary='case_prosecutor',
        backref=u'prosecutor_cases')
...
class Investigation(PlaceMixin, AuditMixin, Model):
    ...
    witness = relationship(
        u'Witnes',
        secondary='investigation_witness',
        backref=u'investigations')
...
class Lawyer(PersonMixin, ContactMixin, AuditMixin, Model):
    ...
    gender1 = relationship(
        u'Gender',
        primaryjoin='Lawyer.gender == Gender.id',
        backref=u'lawyers')
...
class Warder(PersonMixin, ContactMixin, AuditMixin, Model):
    ...
    prison1 = relationship(
        u'Prison',
        primaryjoin='Warder.prison == Prison.id',
        backref=u'warders')
    parent = relationship(
        u'Warder',
        remote_side=[id],
...
class Witnes(PersonMixin, ContactMixin, AuditMixin, Model):
    ...
    gender1 = relationship(
        u'Gender',
        primaryjoin='Witnes.gender == Gender.id',
        backref=u'witness')
